    ...
    def run(self, text, query, log_file=None) -> str:
        ...
        if len(self.enc.encode(self._build_agent_prompt(text, query))) > 12000:
            ...
            completion = client.chat.completions.create(
                ...
                messages= [{'role': 'user', 'content' : self._build_agent_prompt(text, query)}]
            )
...
class ReactPlanner:
    ...
    def step(self) -> None:
        # Think
        self.curr_state = REACTSTATE.THOUGHT
        self.scratchpad += f'\nThought {self.curr_step}:'
        self.scratchpad += ' ' + self.prompt_agent()
        print(self.scratchpad.split('\n')[-1])

        # Act
        self.curr_state = REACTSTATE.ACTION
        ...
        # Observe
        self.curr_state = REACTSTATE.OBSERVATION
        ...
        while True:
            try:
                completion = client.chat.completions.create(
                    extra_body={},
                    model=model,
                    messages= [{'role': 'user', 'content' : self._build_agent_prompt()}]
                )
                content = completion.choices[0].message.content
                return format_step(content, self.curr_step, self.curr_state)
            except:
...
# def format_step(step: str, curr_step: int, curr_state: int) -> str:
#     return step.strip('\n').strip().replace('\n', '')

def format_step(step: str, curr_step: int, curr_state: REACTSTATE) -> str:
    pattern = re.compile(r'\b(action|observation|thought)\s*(\d+)\b', re.IGNORECASE)
    ...
    for match in pattern.finditer(step):
        ...
        if curr_state == REACTSTATE.THOUGHT:
            if (keyword in ["action", "observation"] and number >= curr_step) or \
               (keyword == "thought" and number > curr_step):
                return step[:index].rstrip()

        elif curr_state == REACTSTATE.ACTION:
            if (keyword == "observation" and number >= curr_step) or \
               (keyword in ["thought", "action"] and number > curr_step):
                return step[:index].rstrip()

        elif curr_state == REACTSTATE.OBSERVATION:
            if keyword in ["observation", "thought", "action"] and number > curr_step:
                return step[:index].rstrip()
    return step.strip()
