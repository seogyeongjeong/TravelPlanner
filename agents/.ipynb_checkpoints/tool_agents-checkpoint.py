...
class ReactAgent:
    ...
    def step(self) -> None:
        ...
        self.scratchpad += f'\nThought {self.step_n}:'
        self.react_state = REACTSTATE.THOUGHT
        ...
        # Act
        self.scratchpad += f'\nAction {self.step_n}:'
        self.react_state = REACTSTATE.ACTION
        ...
        self.react_state = REACTSTATE.OBSERVATION
        ...
        while True:
            try:
                ...
                if "gpt" in self.react_name:
                    completion = client.chat.completions.create(
                      extra_body={},
                      model="openai/"+self.react_name,
                      messages= [{'role': 'user', 'content' : self._build_agent_prompt()}]
                    )
                    content = completion.choices[0].message.content
                    return format_step(content, self.step_n, self.react_state)
                else:
                    print(f"Unavailable model name: {self.react_name}")
                    assert(False)
            except:
                # print(f"Error on API call")
